...
Enabling host management features allows you to see the available devices and storage on the physical node as well as browse the node's filesystem. The environment must be [running the Portainer Agent](../../../getting-started/agent.md) to use this functionality, and the root of the host must be bind-mounted to`/host` in the agent deployment:
